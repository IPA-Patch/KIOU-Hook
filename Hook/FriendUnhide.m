        ...
        if (0 /* disabled */
            ...
            if (ptrLooksValid(menuGo)) {
                ...
                if (ptrLooksValid(cloneGo)) {
                    ...
                    (void)spriteOfButton;
                    (void)KIOUEditorApplyTitleSpriteToClone;
...
// resolveIl2cppBridge() / g_unityBaseAddr / p_il2cpp_string_new initialisation
// is required by BOTH distribution modes — the hook bodies and the home-button
// clone path call into the il2cpp bridge regardless of how the install/publish
// side got wired. Pull it out so the JB installer and the binpatch publisher
// share one bootstrap helper.
static void friendUnhide_initBridge(uintptr_t unityBase) {
    resolveIl2cppBridge();
...
void KIOUEditorInstallFriendUnhideHook(uintptr_t unityBase) {
    friendUnhide_initBridge(unityBase);
