...
//
// Migrated from KiouEditor's Sources/KiouEditor/Hook_SyncItemList.m.
//
//   self + 0x20 = updatedSupplyList_ (RepeatedField<SupplyStatus>)
//   SupplyStatus: +0x20 mstSupplyId_(i32), +0x30 isAcquired_(bool),
//                 +0x34 acquiredCount_(i32)
//
...
//     (NEVER touch: +0x44 isFavorite_, +0x45 isSelected_, intimacy{Total,...})
//
//   self + 0x30 = updatedCharacterSkinList_ (RepeatedField<CharacterSkinStatus>)
//   CharacterSkinStatus: +0x18 mstCharacterSkinId_(i32),
//     +0x1C mstCharacterId_(i32), +0x20 isAcquired_(bool),
//     +0x30 acquiredCount_(i32)
//     (NEVER touch: +0x21 isSelected_)
// ===========================================================================

typedef void (*InternalMergeFrom_t)(void *self, void *parseContext);

static InternalMergeFrom_t s_origSyncItemListReply_merge = NULL;
...
// Decoration band check. Currency / character / skin Supplies must NEVER
// be touched, so writes are restricted to these bands.
...
static void hook_SyncItemListReply_merge(void *self, void *parseContext) {
    ...
    if (s_origSyncItemListReply_merge) {
    ...
    @try {
        ...
        bool itemUnlock  = KIOUEditorFeatureEnabled(KIOU_FEATURE_ITEM_UNLOCK);
        // Intimacy-pin is part of Voice Unlock: maxing intimacyLevel makes
        // the CharacterVoicePlayer rule lookups match Level1..Level5 +
        // Complete out of the box, so the two are toggled together.
        bool intimacyMax = KIOUEditorFeatureEnabled(KIOU_FEATURE_VOICE_UNLOCK);
        bool charBypass  = KIOUEditorFeatureEnabled(KIOU_FEATURE_CHAR_BYPASS);
        ...
        // Characters. Walks the list when any of itemUnlock / intimacyMax
        // is on; flips only the fields the active flags own.
        if (itemUnlock || intimacyMax) {
            ...
            if (readRepeatedField(self, 0x28, &charArr, &charCount)) {
                ...
                for (int32_t i = 0; i < charCount; i++) {
                    ...
                    if (intimacyMax) {
                        // CharacterVoicePlayer is built off intimacyLevel;
                        // max it to 5 so every cue rule passes.
        ...
        // Character skins — itemUnlock only.
        if (itemUnlock) {
            void *skinArr = NULL;
            int32_t skinCount = 0;
        ...
        if (charBypass) {
            ...
            KIOUEditorApplyPersistedSelectionToLists(charArr, charCount,
                                                skinArr, skinCount);
        }
    done:;
    } @catch (NSException *e) {
